...
void update() {
    ...
    if ( init_laser && init_robot ) {
        ...
        if ( !current_robot_moving ) {
            ...
            {
                ROS_INFO("robot is not moving and robot was moving");
            }
            else
            {
                ROS_INFO("robot is not moving and robot was not moving");
            }
        }
        else
        {
            ...
            if ( previous_robot_moving )
            {
                ROS_INFO("robot is moving and robot was moving");
            }
            else
            {
                ROS_INFO("robot is moving and robot was not moving");
            }
        }
        previous_robot_moving = current_robot_moving;

        //we search for moving person in 5 steps
        //perform_basic_clustering();//to perform clustering UNCOMMENT only when you have tested the previous function with rosbag and on robair
        //perform_advanced_clustering();//to perform clustering UNCOMMENT only when you have tested the previous function with rosbag and on robair
        //detect_legs();//to detect moving legs using cluster UNCOMMENT only when you have tested the previous function with rosbag and on robair
        //detect_persons();//to detect moving_person using moving legs detected UNCOMMENT only when you have tested the previous function with rosbag and on robair

        // When do we do detection and when do we do tracking ?
        //detect_a_moving_person();  // to detect a moving person UNCOMMENT only when you have tested the previous function with rosbag and on robair
        ...
        if ( !init_robot )
            ROS_INFO("waiting for robot_moving_node: rosrun follow_me robot_moving_node");
...
{
// this function has to be implemented and tested only when the "basic follow me behavior" has been tested and works
